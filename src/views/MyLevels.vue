<template>
  <div class="bg-gradient-to-r from-purple-500 to-pink-500">
    <div
        class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8 animate-fade-down animate-duration-1000">
      ...
      <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
        <div v-for="product in products" :key="product.id" class="group relative">
          ...
          <div class="mt-4 flex justify-between" v-if="product.imageLoaded">
            <div>
              <router-link :to="`/` + product.to">
                <h3 class="text-3xl font-extrabold text-center group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300"
                    :class="product.color">
                  {{ product.name }}
                </h3>
              </router-link>
